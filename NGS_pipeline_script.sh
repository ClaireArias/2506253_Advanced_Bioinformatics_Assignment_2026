...
# If the follwoing tools and dependencies have not been previously installed run once before beginning the pipeline
# ===========================================================================================
...
# Downloaded link can be transfered to ubuntu home directory via FileZilla.
...
# Alongside SnpEff, Snpsift is required
...
echo "This NGS pipeline runs FASTQC, read trimming, alignment, variant calling, and variant annotation on raw sequencing data."
...
# Define the raw FASTQ input file paths for quality control and trimming.
...
# Decompress hg19 reference genome required for FreeBayes
...
# Call variants using Freebayes against hg19 reference genome using FreeBayes
freebayes --bam ~/ngs_pipeline/dnaseq/data/aligned_data/NGS0001_sorted_marked_filtered.bam \
--fasta-reference ~/ngs_pipeline/dnaseq/data/reference/hg19.fa \
--vcf ~/ngs_pipeline/dnaseq/results/NGS0001.vcf
...
# Filter low quality variants  based on quality score, strand balance and read depth.
vcffilter -f "QUAL > 20 & QUAL / AO > 10 & SAF > 0 & SAR > 0 & RPR > 1 & RPL > 1 & DP > 10" ~/ngs_pipeline/dnaseq/results/NGS0001.vcf.gz > ~/ngs_pipeline/dnaseq/results/NGS0001_filtered.vcf

# 11.1 Intersect VCF with BED file

# Use bedtools intersect to only retain variants overlapping target regions within annotation.bed.
bedtools intersect -header -wa -a ~/ngs_pipeline/dnaseq/results/NGS0001_filtered.vcf \
-b ~/ngs_pipeline/dnaseq/data/annotation.bed \
> ~/ngs_pipeline/dnaseq/results/NGS0001_filtered_bedfile.vcf

# Compress and index filtered VCF file
bgzip ~/ngs_pipeline/dnaseq/results/NGS0001_filtered_bedfile.vcf
tabix -p vcf ~/ngs_pipeline/dnaseq/results/NGS0001_filtered_bedfile.vcf.gz
...
~/ngs_pipeline/dnaseq/results/NGS0001_filtered_bedfile.vcf.gz \
> ~/ngs_pipeline/dnaseq/results/NGS0001_filtered_bedfile.avinput

# Run ANNOVAR table function to annotate variants against multiple databases and output a csv file.
~/annovar/table_annovar.pl \
~/ngs_pipeline/dnaseq/results/NGS0001_filtered_bedfile.avinput ~/annovar/humandb/ -buildver hg19 \
-out ~/ngs_pipeline/dnaseq/results/NGS0001_filtered_bedfile -remove \
-protocol refGene,ensGene,clinvar_20180603,exac03,dbnsfp31a_interpro -operation g,g,f,f,f -otherinfo -nastring . -csvout

# 12.2 SnpEff

# NOTE: Should have SnpEff version 4.3 installed  and the snpEff hg19 annotation database downloaded
...
~/ngs_pipeline/dnaseq/results/NGS0001_filtered_bedfile.vcf.gz \
> ~/ngs_pipeline/dnaseq/results/NGS0001_filtered_bedfile_annotated_snpeff.vcf
...
~/ngs_pipeline/dnaseq/results/NGS0001_filtered_bedfile_annotated_snpeff.vcf \
> ~/ngs_pipeline/dnaseq/results/NGS0001_filtered_bedfile_snpsift_prioritised.vcf
